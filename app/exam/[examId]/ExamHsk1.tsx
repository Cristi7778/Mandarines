...
const EXAM_ID = 'exam-hsk1-h10901';
...
const MAX_SCORE    = ALL_QS.length * POINTS_PER_Q;
...
export default function ExamHsk1() {
  ...
  async function finishExam(finalAnswers: Record<number, string>) {
    ...
    setXpEarned(points); setAnswers(finalAnswers); setDone(true);
    await saveExamProgress({ examId: EXAM_ID, score: points, total: MAX_SCORE, completedAt: new Date().toISOString() });
    if (userProgress) await saveUserProgress(updateStreak(addXp(userProgress, points)));
  }
              ...
              {L3_QS.map(lq => (
                <div key={lq.num} className="bg-white rounded-xl border border-gray-100 p-3 flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <LetterPicker letters={L6_LETTERS} selected={matchAnswers[lq.num] ?? null} correctAnswer={lq.answer} confirmed={matchConfirmed} onSelect={v => handleMatchPick(lq.num, v)} />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-2">
              {R2_QS.map(rq => (
                <div key={rq.num} className="bg-white rounded-xl border border-gray-100 p-3 space-y-2">
                  ...
                  <div className="pl-7">
                    <LetterPicker letters={L6_LETTERS} selected={matchAnswers[rq.num] ?? null} correctAnswer={rq.answer} confirmed={matchConfirmed} onSelect={v => handleMatchPick(rq.num, v)} />
                  </div>
                </div>
              ))}
            </div>
